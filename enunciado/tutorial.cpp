...
int stochastic_actions =
    0;  // 0 is deterministic actions, 1 for stochastic actions
    ...
    for (i = 0; i < width_grid; i++) {
        for (j = 0; j < height_grid; j++) {
            ...
            for (k = 0; k < 4; k++) {
                ...
                cout << "Initial Q value of cell [" << i << ", " << j
                     << "] action " << k << " = " << Qvalues[i][j][k] << "\n";
            }
        }
    }
...
int action_selection() {  // Based on the action selection method chosen, it
                          // selects an action to execute next

    if (action_sel ==
        1)  // Greedy, always selects the action with the largest Q value
    {
        return rand() % 4;  // Currently returing a random action, need to code
                            // the greedy strategy
    }

    if (action_sel ==
        2)  // epsilon-greedy, selects the action with the largest Q value with
            // prob (1-exp_rate) and a random action with prob (exp_rate)
    {
        return rand() % 4;  // Currently returing a random action, need to code
...
void move(int action) {
    prev_x_pos = x_pos;  // Backup of the current position, which will become
                         // past position after this method
    prev_y_pos = y_pos;
    ...
    if (stochastic_actions) {
        // Code here should change the value of variable action, based on the
        // stochasticity of the action outcome
    }
    ...
    if (action == 0)  // Up
    {
        if ((y_pos < (height_grid - 1)) &&
            (blocked[x_pos][y_pos + 1] ==
             0))  // If there is no wall or obstacle Up from the agent
        {
    ...
    if (action == 1)  // Right
    {
        if ((x_pos < (width_grid - 1)) &&
            (blocked[x_pos + 1][y_pos] ==
             0))  // If there is no wall or obstacle Right from the agent
        {
    ...
    if (action == 2)  // Down
    {
        if ((y_pos > 0) &&
            (blocked[x_pos][y_pos - 1] ==
             0))  // If there is no wall or obstacle Down from the agent
        {
    ...
    if (action == 3)  // Left
    {
        if ((x_pos > 0) &&
            (blocked[x_pos - 1][y_pos] ==
             0))  // If there is no wall or obstacle Left from the agent
        {
...
void update_q_prev_state()  // Updates the Q value of the previous state
{
    ...
    // Update the Q value of the previous state and action if the agent has not
    ...
    if (!(((x_pos == goalx) && (y_pos == goaly)) ||
        ...
        Qvalues[prev_x_pos][prev_y_pos][action_taken] =
            Qvalues[prev_x_pos][prev_y_pos]
                   [action_taken];  // How should the Q values be updated?

    } else  // Update the Q value of the previous state and action if the agent
...
void Qlearning() {
    ...
    cum_reward = cum_reward +
                 reward[x_pos][y_pos];  // Add the reward obtained by the agent
                                        // to the cummulative reward of the
...
int main(int argc, char* argv[]) {
    ...
    Initialize_environment();  // Initialize the features of the chosen
                               // environment (goal and initial position,
                               // obstacles, rewards)
